...
## 2026-09-16: catch-up to shipped day 17 backend (d4b61c6)
- Copied shipped contract additions into WORK_SPLIT.md without restoring old Stripe scope. CONTRACT_REQUESTS.md now leads with resolved endpoints, current backend release gaps and JD's shared/per-user unread decision.
- Added Versions page: list/metrics, section inspection, note editing and restore as a new version; protected unsaved notes and in-flight restores. Prompt links directly to version history.
- Added Analytics page with date filter, sources/outcomes, conversion groups, estimated revenue, lifetime version metrics and hourly message counts. Added lead-profile notes in the desktop/mobile conversation panel.
- Integrated authenticated EventSource hello/change, coalesced refresh, session/logout cleanup, visible connection state and 30s connected/5s disconnected fallback. Dirty settings/script/version edits are protected. Real incoming sim event produced unread=1 and opening the thread cleared it.
- Expanded JD-only Accounts with platform health/ops, support overview, full audit and guarded deletion. Added account-scoped JSON export and owner deletion UI; profile/vision/transcription/client-value settings; inactive-account AI/send/approve controls disabled.
- 23 frontend regression checks passed; 13 isolated real-backend checks passed including version restore, tenant isolation, analytics, admin-only support and SSE auth. Browser checks passed for notes, analytics, overview, profile empty state, settings and 375px analytics (no page overflow). Original UI/AI ring preserved. No backend changes, deployment, live-provider calls or real-account deletions.
