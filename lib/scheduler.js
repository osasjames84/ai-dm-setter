...
export function createScheduler(deps) {
  ...
  async function followupSweep() {
    const due = deps.followupCandidates(); // rows already filtered by SQL, across ALL accounts
    for (const conv of due) await runAs(conv.account_id, async () => {
      const s = deps.getSettings();           // this conversation's account
      ...
      if (s.kill_switch === '1') return;   // this account's kill switch
      if (seq.length) {
        if (count >= seq.length) return; // sequence done — go dormant, never auto-mark dead
        if (Date.now() < lastAt + stepOffsetMs(seq[count], conv.id + ':' + count)) return; // not due
        ...
        if (!['qualifying', 'qualified', 'booking_sent'].includes(conv.stage)) return;
        const ladder = followupLadder(s);
        if (count >= ladder.length) return; // exhausted → go dormant, never auto-mark dead
        const offsetMs = followupOffsetMs(ladder[count]);
        if (Date.now() < lastAt + offsetMs) return; // not due yet
  ...
  async function bookingSweep() {
    ...
    for (const conv of deps.bookedConvs()) await runAs(conv.account_id, async () => {
      const s = deps.getSettings();           // this conversation's account
      ...
      try {
        ...
        if (!Number.isFinite(callAt)) return;  // no / bad call_time → nothing to schedule
        ...
        if (now > callAt + NOSHOW_GRACE_MS && !sent.includes('noshow')) {
          if (deps.leadSpokeSince && deps.leadSpokeSince(conv.id, conv.call_time)) return; // they showed / replied
    ...
    if (deps.owedReplyConvs) {
      later(() => {
        ...
        for (const c of owed) {
          ...
          runAs(c.account_id, () => onInboundLead(c.id));
        }
      }, FAST() ? 1000 : 10_000);
    }
  }
