 ...
 * counted from the PREVIOUS message. #3/#4 are the long-game re-opens (days
 * later, social-proof territory). A blank/0/invalid step disables that step
...
export function createScheduler(deps) {
  ...
  function maybeFireKeywordTrigger(convId, conv) {
    if (conv.kw_triggered) return false;
    ...
    later(async () => {
      const fresh = deps.getConv(convId);
      if (!fresh || fresh.needs_human || deps.getSettings().kill_switch === '1') return;
  ...
  async function runAutopilotTurn(convId) {
    ...
    try {
      ...
      // action 'send': wait, re-check the snapshot, deliver.
  ...
  async function followupSweep() {
    ...
    for (const conv of due) {
      ...
      if (seq.length) {
        ...
        // AI follow-up ladder (#1 nudge, #2 fresh angle, #3/#4 long-game re-opens
        // days later) — only qualifying/qualified/booking_sent get one.
  ...
  function start() {
    ...
    sweepTimer = setInterval(() => {
      followupSweep().catch((e) => console.error('[sweep]', e.message));
      bookingSweep().catch((e) => console.error('[booking-sweep]', e.message));
    }, FOLLOWUP_SWEEP_MS);
