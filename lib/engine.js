...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    flag_reason_code: {
      type: 'string',
      enum: ['', 'underage', 'blocked_country', 'cant_afford', 'not_interested', 'wrong_fit', 'unqualified', 'cursing', 'disrespectful', 'language', 'generic', 'medical', 'other'],
...
/** The owner's Qualification Criteria as enforce-then-flag rules (empty settings → sensible defaults). */
function qualificationBlock(s) {
  const bc = String(s.blocked_countries || '').trim();
  ...
  if (bc) lines.push(`- Blocked locations: ${bc}. If clearly from one → run the REGION HANDOFF in SOFT DISQUALIFICATION (above): NEVER disclose the region policy, ask ONE budget-probe question in your own words, and set needs_human = true with flag_reason_code "blocked_country" so the owner's human setter takes over. no letdown, no redemption, no free-guide redirect — the human owns the close.`);
...
const FLAG_REASON_DESC = {
  ...
  blocked_country: "they're from a region your Qualification Criteria exclude — silent handoff, not a decline (you asked one budget-probe question; the human setter takes the close)",
 ...
 *  flagging block ever presents as scenarios (plus the always-on 'medical').
 *  'other' is deliberately absent: it exists for engine errors and manual
 *  dashboard flags, and a stale/legacy `other: true` in settings must never
 *  re-open the old catch-all that flagged refunds, bot suspicion, etc. */
const TOGGLEABLE_FLAGS = ['underage', 'blocked_country', 'cant_afford', 'not_interested', 'wrong_fit', 'unqualified', 'cursing', 'disrespectful', 'language', 'generic'];
/** The owner's enabled flag toggles, restricted to real toggleable codes.
 *  Falls back to cursing/disrespectful until configured. */
function enabledFlags(s) {
  let enabled; try { enabled = JSON.parse(s.flag_enabled || '{}'); } catch { enabled = {}; }
  if (!enabled || typeof enabled !== 'object' || !Object.keys(enabled).length) {
    enabled = { cursing: true, disrespectful: true };
  }
  return TOGGLEABLE_FLAGS.filter((k) => enabled[k]);
}

/**
 * When to set needs_human — the owner's flag_enabled toggles plus ONE always-on
 * safety case (a serious health situation). NOTHING else flags: the setter's
...
function flaggingBlock(s) {
  ...
  lines.push('ONLY these scenarios flag — this list is EXHAUSTIVE:');
  lines.push(`- medical (always on) — ${FLAG_REASON_DESC.medical}. Send ONE caring, human message first (acknowledge, suggest they see a professional where fitting) and set needs_human = true with flag_reason_code "medical". Normal aches, old injuries, allergies, meds, "bad knees" etc. are routine coaching intake — NOT this.`);
...
function buildSystemPrompt(s) {
  ...
  return [
    ...
    `When a lead hits an AGE or AFFORDABILITY filter, you NEVER go silent and you NEVER just flag-and-vanish. let them down softly, honestly, and in ONE warm message. BLOCKED REGION is handled completely differently — see the REGION HANDOFF rule below; it is NOT a letdown.`,
    `REGION INFERENCE: treat STRONG regional signals as a region hit even when no country is stated — their currency (naira, cedis), their cities (lagos, abuja, accra), or them naming prices in a blocked region's money. when the signal is STRONG, run the REGION HANDOFF below. when it's WEAK or ambiguous, ask casually where they're based BEFORE deep qualifying — ONE light question, not an interrogation.`,
    `REGION HANDOFF (silent — never disclose the region policy): on a region hit (stated or strongly inferred), do NOT decline, do NOT mention regions, countries, or that you "don't support" anywhere, and do NOT talk price. ask ONE budget-probe question in your own words, spirit of "how much would you be willing to budget for 1-1 coaching a month?", AND set needs_human = true with flag_reason_code "blocked_country" and reason like "blocked region: handoff for low-ticket close". you send that ONE question and you are DONE — the owner's human setter takes over the close from there. no letdown, no goodbye, no free-guide redirect, no redemption on your side.`,
    ...
    `IF THEY PUSH BACK OR ASK WHY (age or affordability only): be honest and kind, nothing personal. never invoke this for a region hit — region gets no explanation from you at all.`,
    `IF THEY ASK HOW MUCH on the AFFORDABILITY path (including the BUDGET LADDER's low-budget rescue rung): this is the ONE exception to the never-state-prices rule — give the soft range, spirit of "it's between £300-500 or so a month, but the real number depends on the plan we'd build out for you". never a hard quote. this exception does NOT apply to the region path — the AI never discusses price on a region hit; the human handles it.`,
...
export async function generateMove(settings, conversation, history, options = {}) {
  ...
  // first-ask bot suspicion) despite the prompt; a flag parks the thread and
  // bleeds the lead, so enforce the list deterministically: a needs_human whose
  // flag_reason_code is not allowed (owner toggles + always-on medical +
  // blocked_country when regions are configured) is downgraded to a normal send.
  // Only applies when there ARE messages — a flag with no reply text (engine
  // errors return earlier) has nothing to send, so parking stays correct.
  const allowedCodes = new Set([...enabledFlags(settings), 'medical']);
  if (String(settings.blocked_countries || '').trim()) allowedCodes.add('blocked_country');
