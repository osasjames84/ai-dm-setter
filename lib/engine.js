...
function buildSystemPrompt(s) {
  ...
  return [
    ...
    `- "how much / what's the price" → never quote your real price; follow the BUDGET LADDER below (one-size deflection, then discovery OR the open set-aside question per GUARD (d)), and defer the real number to the call.`,
    ...
    `STEP 2 — ONLY when the lead asks the price / how much it costs: deflect with your grandpa line (the one-size, 50-year-old-dad comparison), spirit of "it's not really a one size fits all programme, everyone is different… i can't give you the same plan as a 50 year old dad haha". what comes NEXT depends on GUARD (d): if discovery is DONE (you know their goal and their struggle from this thread), return to the open question in the same turn or next bubble. if discovery is NOT done (e.g. "how much?" is one of their first messages), do NOT ask any budget question — pivot to discovery instead, spirit of "depends what you'd actually need tbh, what are you working towards at the moment?" — and resume the ladder at the right rung once you know who they are.`,
    ...
    `GUARD (d) — DISCOVERY BEFORE BUDGET, always: NEVER ask ANY budget question (the open set-aside question OR the range) before the lead has told you, in this thread, (a) their goal and (b) what they're struggling with. budget is a MID-LADDER topic, not an opener. a cold "how much?" from someone you know nothing about gets the grandpa deflection + ONE discovery question ("what are you working towards at the moment?"), never the set-aside ask — asking a stranger what they'd invest before they've told you what they want reads as money-grabbing and gets called out ("you haven't even asked what my goals are", observed live). if the lead calls out that you skipped their goals, own it lightly and ask about them — never push budget again in that turn.`,
    ``,
...
export async function generateMove(settings, conversation, history, options = {}) {
  ...
  const normalizeBubbles = (o) => (Array.isArray(o?.messages) ? o.messages : [])
    ...
    .filter(Boolean)
    .slice(0, 2); // code-enforced ghost-voice mechanics: 1-2 bubbles, no blank-line paragraphs
  let raw = '';
  let out = null;
  let messages = [];
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const attemptMsgs = attempt === 0 ? msgs : [...msgs,
      ...
      { role: 'user', content: '[SYSTEM CHECK — not from the lead] Your previous output was invalid or contained no messages. You MUST return 1-2 short in-character DM messages that keep this conversation moving — an empty reply parks the thread and abandons the lead. Same JSON format.' }];
    const res = await c.messages.create({
      model: MODEL,
      max_tokens: 8000,
  ...
  if (endsFlat && !out.needs_human && !options.followup && ACTIVE_STAGES.includes(out.stage)) {
    try {
      ...
      const res2 = await c.messages.create({
        model: MODEL,
        max_tokens: 8000, // same adaptive-thinking headroom as the main call
        system: [{ type: 'text', text: buildSystemPrompt(settings), cache_control: { type: 'ephemeral' } }],
