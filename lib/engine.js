...
const TOGGLEABLE_FLAGS = ['underage', 'cant_afford', 'not_interested', 'wrong_fit', 'unqualified', 'cursing', 'disrespectful', 'language', 'generic'];
...
export async function generateMove(settings, conversation, history, options = {}) {
  ...
  // first-ask bot suspicion, and — the case that got this removed entirely — a
  // lead using the Nigerian/Yoruba slang "omo" wrongly read as a region signal
  // and silently handed off); a flag parks the thread and bleeds the lead, so
  // enforce the list deterministically: a needs_human whose flag_reason_code is
  // not allowed (owner toggles + always-on medical) is downgraded to a normal
  // send. Only applies when there ARE messages — a flag with no reply text
  // (engine errors return earlier) has nothing to send, so parking stays correct.
