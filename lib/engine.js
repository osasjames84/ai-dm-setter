...
export async function generateMove(settings, conversation, history, options = {}) {
  ...
  // hit and fixed). On a truncated/unparseable result, retry ONCE — the system
  // prompt is cache-hit so the retry is cheap — and only flag if both attempts fail.
  let raw = '';
  let out = null;
  for (let attempt = 0; attempt < 2 && !out; attempt++) {
    ...
    if (!out && attempt === 0) console.log('[engine] truncated/unparseable move output — retrying once');
  }
  if (!out) {
