 ...
 */
function extractMessageEvent(ev) {
  ...
  const businessId = String(process.env.IG_BUSINESS_ID || '');
...
export function igParseInbound(body) {
  ...
  for (const entry of body?.entry || []) {
    for (const ev of entry.messaging || []) {
      const parsed = extractMessageEvent(ev);
      if (parsed) out.push(parsed);
    }
    for (const change of entry.changes || []) {
      if (change.field !== 'messages') continue;
      const parsed = extractMessageEvent(change.value);
      if (parsed) out.push(parsed);
    }
  }
...
export async function igProfile(senderId) {
  try {
    const res = await fetch(`${GRAPH}/${senderId}?fields=name,username,profile_pic&access_token=${process.env.IG_PAGE_TOKEN}`);
    if (!res.ok) {
      const body = await res.text();
