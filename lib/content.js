  ...
  properties: {
    ideas: {
      ...
      items: {
        type: 'object',
        properties: {
          title: { type: 'string', description: 'the whole idea in ONE self-contained line — a piece of content the coach could film or write from this line alone (e.g. "why every tiktok gives you a different calorie number and who\'s actually right")' },
...
export async function analyzeDms(leadMessages) {
  ...
  try {
    const res = await c.messages.create({
      ...
      max_tokens: 8000,
      system: [
        'You are a content strategist mining a UK fitness coach\'s Instagram DMs from real leads.',
        ...
        'The audience is lowercase UK-casual; keep their voice, do not sanitise it.',
      ].join(' '),
...
export async function generateIdeas(analysis, existingTitles = []) {
  ...
  try {
    const res = await c.messages.create({
      ...
      max_tokens: 12000,
      system: [
        'You are a content strategist for a UK fitness coach. From the supplied DM analysis, produce a PLAIN LIST of 24 possible content ideas mined from his DMs.',
        'Each idea is ONE self-contained line describing a piece of content he could make — concrete enough to film or write from that line alone (e.g. "why every tiktok gives you a different calorie number and who\'s actually right", "the real cost of restarting your diet 4 times a year", "what a £300/month client actually gets vs what people assume").',
        ...
        'Style: UK fitness audience, casual, lowercase. No em dashes. No hashtags.',
      ].join(' '),
