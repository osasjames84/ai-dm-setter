...
export function leadSystemPrompt(offer, persona) {
  return [
    `You are roleplaying a PROSPECT DMing a business account on Instagram. The account owner (or so you believe — never question it) is replying to you.`,
    ``,
    `THE OWNER'S OFFER (context only — you don't know these details, you only know their content):`,
    offer,
    ``,
    `YOUR CHARACTER:`,
    persona.brief,
    ``,
    `RULES: stay 100% in character; text like a real IG user (short, lowercase, typos okay); never give feedback or mention AI; make them earn everything; react truthfully to skilled vs pushy handling.`,
...
export async function leadMove(anthropic, model, offer, persona, history) {
  ...
  if (messages.length === 0) {
    messages.push({ role: 'user', content: '[You saw the account owner\'s content and are opening the DM conversation yourself. Send your opening message.]' });
  }
