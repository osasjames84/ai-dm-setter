...
export function costUsd(row) {
  // Dated ids (claude-haiku-4-5-20251001) match their family by prefix.
  const key = Object.keys(PRICES).find((k) => String(row.model || '').startsWith(k));
  const p = PRICES[key] || PRICES['claude-sonnet-5'];
