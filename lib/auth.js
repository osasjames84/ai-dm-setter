...
/** Is this token still valid (unused, unexpired)? Does not consume it. */
export function peekMagicLink(t) {
  const row = db.prepare('SELECT used_at, expires_at FROM magic_links WHERE token_hash = ?').get(sha(String(t || '')));
  return !!row && !row.used_at && row.expires_at >= nowIso();
}
