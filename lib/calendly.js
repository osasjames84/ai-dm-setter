...
const _caches = new Map(); // accountId → { at, text }
export async function refreshCalendly(token, opts = {}, accountId = 'acc_1') {
  if (!token) { _caches.delete(accountId); return; }
  try {
    const avail = await fetchAvailability(token, opts);
    _caches.set(accountId, { at: opts.now || Date.now(), text: availabilityText(avail) });
  } catch (e) { /* keep last good cache; availability is best-effort */ }
}

/** Synchronous cached availability snippet for engineSettings(). '' when none or stale (> 2h). */
export function calendlyText(accountId = 'acc_1') {
  const c = _caches.get(accountId);
